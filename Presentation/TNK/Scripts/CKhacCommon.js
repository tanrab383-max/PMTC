...
function CheckValidatePhieuChi() {
    ...
    if ($("#maLoaiPhieu").val() == "CHIHO"
        && parseInt($("#Item_ConLai").val().split(",").join("")) > 0
    ) {
        toastWarning("Số tiền thanh toán phải bằng số tiền chi.");
        return false;
    }
    var lst = $(".number");
...
function TinhConLai() {
    ...
    var total = parseInt($("#Item_TongCong").text().split(",").join(""));
    var TongSoTienChi = $("#TongCongThanhToan").val() == "" ? 0 : parseInt($("#TongCongThanhToan").val().split(",").join(""));
    var rest = total - TongSoTienChi;
