...
function CheckValidatePhieuChi() {
    ...
    var invalidSL = $('input[name^="ListCTNKPT"][name$=".SoLuong"]')
        .filter(function () {
            var val = $.trim(this.value);
            return val === '' || isNaN(val) || Number(val) <= 0;
        });

    if (invalidSL.length > 0) {
        toastWarning('Số lượng phải là số và lớn hơn 0');
        invalidSL.first().focus();
        return false; // ✅ hợp lệ vì nằm trong function
    }

    if ($("#NCC").val() == "") {
        toastWarning("Vui lòng chọn nhà cung cấp");
        return false;
    }
    ...
    if (($("#Item_TongCong").val() != undefined
        && $("#TienNhapKho").text() != undefined
         && $("#TienNhapKho").text() != ""
        && parseInt($("#Item_TongCong").val().split(",").join("")) != parseInt($("#TienNhapKho").text().split(",").join("")))
    ) {
    ...
    if ($("#Item_ConLai").val() != undefined && parseInt($("#Item_ConLai").val().split(",").join("")) < 0) {
        toastWarning("Số tiền còn lại không được nhỏ hơn 0.");
        return false;
    }
    if ($("#SoChungTu").val() != undefined && ($("#SoChungTu").val() == null || $("#SoChungTu").val() == ""))  {
        toastWarning("Vui lòng nhập số chứng từ");
        return false;
    }
    var lst = $(".number");
