...
function CheckValidatePhieuChi() {
    debugger;
    //alert($("#Item_TongCong").val());
    //alert($("#TongCongThanhToan").val());
    //alert(total);
    //if ($("#TongCongThanhToan").val() == "")
    //    calTotal();
    //alert($("#item_GiaVon").val());
    //trúc thêm, số tiền không được <= 0
    if ($("#idTongCong").val() != undefined && $("#idTongCong").val() <= 0) {
        toastWarning("Tổng cộng không được bằng 0");
        return false;
    }
    ...
    if ($("#LDC").val() != undefined && $("#LDC").val() == "") {
        toastWarning("Vui lòng nhập lý do chi");
        return false;
    }

    //phần về ràng buộc phòng ban chi tổng hợp
    if ($("#BOPHAN").val() != undefined && $("#BOPHAN").val() == "") {
        toastWarning("Vui lòng nhập bộ phận chi");
        return false;
    }
    ...
    //}
    debugger
    if ($("#type").val() != undefined && $("#type").val() == "CTOHO") {
        var TongChiCoc = $("#TongSoTienChi").val() == "" ? 0 : parseInt($("#TongSoTienChi").val().split(",").join(""));
        var TongThanhToan = $("#TongCongThanhToan").val() == "" ? 0 : parseInt($("#TongCongThanhToan").val().split(",").join(""));
        var TongThanhToan = TongChiCoc + TongThanhToan;
        let number = parseInt($("#item_ConLai").val().split(",").join(""));
        if (number > 0 && TongThanhToan > 0) {
            toastError("Bạn không thể vừa nhập thanh toán vừa treo nợ đối với chi phí. Vui lòng liên hệ phòng KTNB");
            return false;
        }
    }
    var lst = $(".number");
...
function TinhConLai() {
    ...
    var TongSoTienChi = $("#TongSoTienChi").val();
    var TongCongThanhToan = $("#TongCongThanhToan").val();

    if (TongSoTienChi != "")
        TongSoTienChi = parseInt($("#TongSoTienChi").val().split(",").join(""));
    else
        TongSoTienChi = 0;
    if (TongCongThanhToan != "")
        TongCongThanhToan = parseInt($("#TongCongThanhToan").val().split(",").join(""));
    else
        TongCongThanhToan = 0;
    var TongCong = TongSoTienChi + TongCongThanhToan;
    $("#idTongCong").val(TongCong.toLocaleString('en-us'));
    $("#idTongCong").html(TongCong.toLocaleString('en-us'));
}
function TinhConLai_v2() {
    debugger
    var TongTien = $("#TongTien").val().split(",").join("");
    var TongThanhToan = $("#TongSoTienChi").val().split(",").join("") + $("#TongCongThanhToan").val().split(",").join("");
    var ConLai = TongTien - (TongThanhToan == "" ? 0 : parseInt(TongThanhToan));
    $("#item_ConLai").val(ConLai.toLocaleString("en-us"));
}
