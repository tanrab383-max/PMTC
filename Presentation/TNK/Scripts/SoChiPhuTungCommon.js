...
function CalThanhToan(a) {
    ...
    setTimeout(function () {
        ...
        if ($("#Item_TongCong").val() != undefined && $("#Item_ConLai").val() != undefined) {
            ...
            var TongSoTienChi = $("#TongSoTienChi").val() == "" ? 0 : parseInt($("#TongSoTienChi").val().split(",").join(""));
            //alert(TongSoTienChi);
            var rest = buyprice - total - TongSoTienChi;
...
function CheckValidatePhieuChi() {
    ...
    if ($("#Item_ConLai").val() != undefined && parseInt($("#Item_ConLai").val().split(",").join("")) < 0) {
        toastWarning("Còn lại không được nhỏ hơn 0");
        return false;
    }
